...
var unitCheckbox = document.querySelector("#unitCheckbox");
...
async function retrieveWeather(unit, city) {
  const vUnit = unit === "I" ? "imperial" : "metric";
  const lat = city.lat;
  const lon = city.lon;
  ...
  const weatherURL =
    ...
    "&lon=" +
    lon +
    "&exclude=hourly,minutely&appid=8793424064af88ecfa3af87c2352b444&units=" +
    vUnit;
...
function populateHistory(cities) {
  ...
  currntCity = historyList[0];

  retrieveWeather(displayUnit, historyList[0]);
}
...
function refreshWeatherPanel(city, weatherJson) {
  ...
  const tempUnit = displayUnit === "I" ? "F" : "C";
  const windSpeedUnit = displayUnit === "I" ? "MPH" : "m/s";

  currentTempElement.innerHTML = `Temp: ${weatherJson.current.temp.toFixed(
    1
  )}&deg;${tempUnit}`;

  currentWindElement.innerHTML = `Wind: ${weatherJson.current.wind_speed} ${windSpeedUnit}`;
...
function refreshWeatherPanelForecast(weatherJson) {
  var cards = weatherJson.daily.map((day) => {
    ...
    return `<div class="card">
                ...
                <div class="weatherIcon"><img alt="weather" src="http://openweathermap.org/img/wn/${
                  day.weather[0].icon
                }.png" /></div>
                <div class="futureWeatherText">Temp: ${day.temp.day.toFixed(
                  1
                )}&deg;${tempUnit}</div>
                <div class="futureWeatherText">Wind: ${
                  day.wind_speed
                } ${windSpeedUnit}</div>
...
function getWeather(city, lat, lon) {
  ...
  cityCOR.lat = lat;
  cityCOR.lon = lon;
  retrieveWeather(displayUnit, cityCOR);
}
...
function changeUnit() {
  displayUnit = unitCheckbox.checked ? "I" : "M";

  retrieveWeather(displayUnit, currntCity);
}
